...
public class GoldAlignDetector extends DogeCVDetector {
    ...
    // Gold position Enum and result
    public enum Pos{LEFT, MIDDLE, RIGHT};
    public Pos goldFieldPos;
    ...
    public Mat process(Mat input) {
        yellowFilter = new LeviColorFilter(LeviColorFilter.ColorPreset.YELLOW, AutonFunctions.filterThreshold);
        ...
        // Vars to determine Gold position
        double rightBound = alignXMin;   //280;  Right bound uses smaller number
        double leftBound = alignXMax;  //360;    because phone is upside down
        ...
        if(bestRect != null){
            ...
            // Check mineral position - Right is smaller number because phone is upside down
            if (xPos < rightBound) {
                goldFieldPos = Pos.RIGHT;
            }
            else if (xPos > leftBound) {
                goldFieldPos = Pos.LEFT;
            }
            else
                goldFieldPos = Pos.MIDDLE;
        ...
        //Print result
        Imgproc.putText(displayMat, "Image Width: " + getAdjustedSize().width, new Point(10,getAdjustedSize().height - 90),0,1, new Scalar(255,255,0),1);
        Imgproc.putText(displayMat,"isFound: " + found,new Point(10,getAdjustedSize().height - 60),0,1, new Scalar(255,255,0),1);
        Imgproc.putText(displayMat,"isAligned: " + aligned,new Point(10,getAdjustedSize().height - 30),0,1, new Scalar(255,255,0),1);
    ...
    public Pos getFieldPos() {return goldFieldPos;}
}
